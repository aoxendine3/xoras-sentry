        ...
        walk.simple(ast, {
            MemberExpression(node) {
                ...
                if (node.object.type === 'MemberExpression' && node.object.object.name === 'process' && node.object.property.name === 'env') {
                    ...
                    // Handle dynamic: process.env[`${'STRIPE'}_KEY`]
                    if (node.property.type === 'TemplateLiteral') {
                        let resolvedKey = '';
                        node.property.quasis.forEach((quasi, i) => {
                            resolvedKey += quasi.value.cooked;
                            // Note: Simple resolution of literal expressions only
                            if (node.property.expressions[i] && node.property.expressions[i].type === 'Literal') {
                                resolvedKey += node.property.expressions[i].value;
                            }
                        });
                        if (resolvedKey) vars.add(resolvedKey);
                    }
                }
            },
...
async function scanSource(dir, vars = new Set(), hardcodedSecrets = [], depth = 0, root = null) {
    ...
    // 3. Schema Validation Pass (High Integrity)
